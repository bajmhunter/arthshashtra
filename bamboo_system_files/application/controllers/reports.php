...
class Reports extends MY_Controller {
	...
	function index()
	{
		...
		for ($i=1; $i<13; $i++)
		{
			...
			$data['month_invoices'][$i] = round($this->reports_model->getSummaryData(date('Y'). "-$monthnum-01", date('Y')."-$monthnum-31")->amount);

			// tax1
			($i < 10) ? $monthnum = "0$i" : $monthnum=$i;
			$data['month_tax1'][$i] = round($this->reports_model->getSummaryData(date('Y'). "-$monthnum-01", date('Y')."-$monthnum-31")->tax1_collected);

			// tax2
			($i < 10) ? $monthnum = "0$i" : $monthnum=$i;
			$data['month_tax2'][$i] = round($this->reports_model->getSummaryData(date('Y'). "-$monthnum-01", date('Y')."-$monthnum-31")->tax2_collected);
		}
		...
		$data['yearToDateCount'] = $this->reports_model->getInvoiceDateRange(date('Y').'-01-01', date('Y').'-12-31')->num_rows() . ' ' . $this->lang->line('reports_invoices_issued_year');

		$current_year_data = $this->reports_model->getSummaryData(date('Y').'-01-01', date('Y').'-12-31');
	...
	function dates()
	{
		...
		elseif ($detailed_data->num_rows() > 0)
		{
			...
			else
			{
				...
				$this->table->add_row('<strong>Total</strong>', '<strong>' . $this->settings_model->get_setting('currency_symbol') . number_format($detailed_data_summary->amount, 2, $this->config->item('currency_decimal'), ''), '<strong>' . $this->settings_model->get_setting('currency_symbol') . number_format($detailed_data_summary->tax1_collected, 2, $this->config->item('currency_decimal'), '') . '</strong>', '<strong>' . $this->settings_model->get_setting('currency_symbol') . number_format($detailed_data_summary->tax2_collected, 2, $this->config->item('currency_decimal'), '') . '</strong>');
			}
		...
		{
			$data['data_table'] = '<p class="error">There is no data available for those dates.</p>';
		}
