	...
	<?php echo form_open('invoices/email/' . $row->id, array('id' => 'emailInvoice', 'name' => 'emailInvoice'), array('invoice_number'=>$row->invoice_number, 'isAjax'=>'false'));?>
			...
			<fieldset id="recipients">
				...
				<p>
					...
					<?php endforeach;?>
					<br /><label for="primary_contact"><input name="primary_contact" id="primary_contact" type="checkbox" value="y" /><em>blind copy me on this email</em></label>
				</p>
			</fieldset>
			...
			foreach($invoiceHistory->result() as $row): ?>
				<div style="clear:left; margin: 10px 0;">
					...
					<?php if ($row->contact_type == 2): ?>
						...
						<?php else: ?>
						<div class="comment"><p class="commentintro"><?php echo $this->lang->line('invoice_sent_to');?> <p><?php echo implode(", ", unserialize($row->clientcontacts_id));?></p><?php echo auto_typography(str_replace('\n', "\n", $row->email_body));?></p></div>
					<?php endif; ?>
