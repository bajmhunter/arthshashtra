...
<?php 
		...
		// localized month
		$display_date = $this->lang->line('cal_' . strtolower(date('F', mysql_to_unix($row->dateIssued))));
		// day and year numbers
		$display_date .= date(' j, Y', mysql_to_unix($row->dateIssued));

?>
	<tr>
		<td><?php echo anchor('invoices/view/'.$row->id, $row->invoice_number);?></td>
		<td><?php echo anchor('invoices/view/'.$row->id, $display_date);?></td>
		<td class="cName"><?php echo anchor('invoices/view/'.$row->id, $row->name);?></td>
